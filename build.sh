...
[ $GPUMODEL = "r15p0" ] && cat $RDIR/arch/arm64/configs/r15p0_defconfig >> $RDIR/arch/arm64/configs/$KERNELCONFIG
...
echo "- Building system-r22p0.tar.xz file ..."
cd $ZIPDIR/tgpkernel/system-r22p0
tar -cf - system/ | xz -9 -c - > system-r22p0.tar.xz
mv -f system-r22p0.tar.xz $ZIPDIR/tgpkernel/files/system-r22p0.tar.xz
cd ..
rm -rf $ZIPDIR/tgpkernel/system-r22p0
echo "- Building system-r15p0.tar.xz file ..."
cd $ZIPDIR/tgpkernel/system-r15p0
tar -cf - system/ | xz -9 -c - > system-r15p0.tar.xz
mv -f system-r15p0.tar.xz $ZIPDIR/tgpkernel/files/system-r15p0.tar.xz
cd ..
rm -rf $ZIPDIR/tgpkernel/system-r15p0
...
# Build S7 r22p0 .img files
echo ""
echo "Building S7 r22p0 .img files"
echo "----------------------------"
echo ""
git checkout tw80 -f
MODEL=S7
...
export KERNELTITLE=$KERNELNAME.$MODEL.$ANDROIDVERSION.$VERSION_NUMBER.r22p0
LOGFILE=$OUTPUT/build-s7-r22p0.log
BUILD_PREPARE
FUNC_BUILD_BOOTIMG
BUILD_CLEAN
...
MODEL=S7E
export KERNELTITLE=$KERNELNAME.$MODEL.$ANDROIDVERSION.$VERSION_NUMBER.r22p0
LOGFILE=$OUTPUT/build-s7e-r22p0.log
BUILD_PREPARE
FUNC_BUILD_BOOTIMG
BUILD_CLEAN
...
GPUMODEL=r15p0
export ANDROIDVERSION=O
export KERNELTITLE=$KERNELNAME.$MODEL.$ANDROIDVERSION.$VERSION_NUMBER.r15p0
LOGFILE=$OUTPUT/build-s7-r15p0.log
BUILD_PREPARE
FUNC_BUILD_BOOTIMG
BUILD_CLEAN
...
MODEL=S7E
export KERNELTITLE=$KERNELNAME.$MODEL.$ANDROIDVERSION.$VERSION_NUMBER.r15p0
LOGFILE=$OUTPUT/build-s7e-r15p0.log
BUILD_PREPARE
FUNC_BUILD_BOOTIMG
BUILD_CLEAN
