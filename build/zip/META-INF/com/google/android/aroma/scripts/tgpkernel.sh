...
if [ $OPTION == "setup_extract" ]; then
	...
	if grep -q install=1 $AROMA/r15p0.prop; then
		tar -Jxf system-r15p0.tar.xz
	else
		tar -Jxf system-r22p0.tar.xz
	fi
	exit 10
fi
...
if [ $OPTION == "kernel_patch" ]; then
	...
	if grep -q install=1 $AROMA/g930x.prop; then
		if grep -q install=0 $AROMA/r15p0.prop; then
